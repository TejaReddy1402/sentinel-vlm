...
def _read_band(href: str, bbox_wgs84: List[float], target_px: int = 512) -> np.ndarray:
    with rasterio.Env(
        ...
        with rasterio.open(href) as src:
            ...
            # Clip to actual tile extent — removes black zero-padded borders
            tile_window = rasterio.windows.Window(0, 0, src.width, src.height)
            window = window.intersection(tile_window)
